...
% =========================================================================
   ...
   function plot_triple_simulation(data,data2,data3)
...
%% load inputs
   ...
   lbd3   = data3.lbd                                                     ; % lower bands for simulated debt paths
   ubd3   = data3.ubd                                                     ; % upper bands for simulated debt paths
   lbr3   = data3.lbr                                                     ; % lower bands for simulated fiscal reaction function paths
   ubr3   = data3.ubr                                                     ; % upper bands for simulated fiscal reaction function paths 
   pfd3   = data3.pfd                                                     ; % debt point forecasts
   pfr3   = data3.pfr                                                     ; % fiscal reaction function point forecasts
...
%  confidence intervals       
   for j=nb:-1:1
       shadedplot(fdates',lbd3(1:h+1,j)',ubd3(1:h+1,j)',areac3)           ;
       areac3 = areac3 - areas                                            ;
       hold on
   end     
...
%  point forecasts
 ...
 %  point forecasts
   pl = plot(fdates,pfd2,'g-','linewidth',1.3)                            ;                                                       
        hold on    
        
%  point forecasts
   pl = plot(fdates,pfd3,'b-','linewidth',1.3)                            ;                                                       
        hold on          
...
%  title  
   title('Simulated paths for $d_{t}$','fontsize',14,'interpreter','latex')               
...
%  point forecasts
   pl = plot(fdates,pfd2,'r-','linewidth',1.3,'HandleVisibility','off')   ;                                                       
        hold on       
...
%  point forecasts
   pl = plot(fdates,pfd,'b-','linewidth',1.3,'HandleVisibility','off')    ;                                                       
        hold on 
...
%  point forecasts
   pl = plot(fdates(1:end),pfr(1:end),'b-','linewidth',1.3, 'HandleVisibility','off');                                                       
        hold on  
...
%  point forecasts
   pl = plot(fdates(1:end),pfr2(1:end),'r-','linewidth',1.3,'HandleVisibility','off');                                                       
        hold on      
...
%  axis and labels  
   ...
   ylim([min([pfr(1:h+1);lbr(1:h+1,nb)-0.5])                                  ...
         min(max([pfr;ubr(1:h+1,nb)+0.5]),10+data.pb0)])                  ;      
   grid on         
   set(gca,'fontsize',9,'ticklabelinterpreter','latex'                    ... 
          ,'ticklabelinterpreter','latex')                                ;
